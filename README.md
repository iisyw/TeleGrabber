...
- **混合下载模式**：默认使用标准 Bot API (仅限 20MB 以内)，配置 User API 后支持大文件下载和 `/link` 消息链接下载
- **SQLite 数据库**：持久化存储所有媒体元数据，支持全局去重与跨会话追踪
- **极速并发**：小文件 (<20MB) 多线程并发下载，大文件使用串行接力
- 自动检测并保存正确的媒体格式（jpg、png、gif、webp、mp4、avi 等）
- **实时状态更新**：进度条精确到百分比，并用不同 emoji 区分 Bot API（✅/🔽）与 User API（🟢/☁️）下载通道
- **智能去重与安全删除**：自动跳过库中已存在的资源；“删除”操作仅撤销本次下载，绝不触及历史存量数据
- **单条 / 媒体组操作按钮**：下载完成后提供重新下载、强制重下、删除等即时操作
...
- `/link 消息链接` 或 `/link username 消息ID`：通过 User API 下载当前账号可访问消息中的媒体

所有媒体文件将保存在 `downloads` 目录下（或你在 `.env` 中指定的其他目录），按来源统一归类（详见“文件夹组织结构”）。
...
媒体组进度条会用不同 emoji 区分下载通道：Bot API 使用 ⏳/🔽/✅/❌，User API 使用 🕓/☁️/🟢/🔴，一眼看出每项实际走哪种下载方式。
...
- 删除后会保留 ♻️ 重新下载按钮，方便删了再下回来；重新下载在原消息上原地更新进度与结果。
